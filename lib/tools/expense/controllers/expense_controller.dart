...
class ExpenseController extends ChangeNotifier {
  ...
  Future<bool> add(ExpenseRecord record) async {
    ...
    final success = await _persist(
      rollback: () => _records.remove(assigned),
      affectedMonths: [record.date],
    );
    if (success) {
  ...
  Future<bool> update(ExpenseRecord changed) async {
    ...
    return _persist(
      ...
      affectedMonths: [original.date, storedChange.date],
    );
  }
  ...
  Future<bool> _persist({
    ...
    required Iterable<DateTime> affectedMonths,
  }) async {
    if (_saving) {
      rollback();
    ...
    try {
      await _storage.save(_records);
      try {
        await _syncMonths(affectedMonths);
      } on PublicExportException catch (exception) {
  ...
  Future<void> _syncAllMonthFiles() async {
    final months = <DateTime>{
      DateTime(DateTime.now().year, DateTime.now().month),
      for (final item in _records) DateTime(item.date.year, item.date.month),
    };
    await _syncMonths(months);
  }

  /// 日常增删改只刷新受影响月份，避免账目增长后反复扫描全部历史数据。
  Future<void> _syncMonths(Iterable<DateTime> affectedMonths) async {
    final months = {
      for (final month in affectedMonths) DateTime(month.year, month.month),
    };

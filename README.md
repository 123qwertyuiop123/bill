...
一款简洁、本地优先的 Flutter 多功能工具 App。当前提供 51 个离线工具，并完整保留原有收支账本、分类统计、多 TXT 账本切换和 Android 公共目录自动同步功能。
...
- 基础计算器（不执行输入代码）
- 长度、重量、温度和数据容量单位换算，区分十进制 KB 与二进制 KiB
- 日期间隔和日期推算
...
- 4/5 色环与三位/四位 ASCII 数字贴片电阻解码；色环显示误差与范围，不替代实测
- 二次方程实根、复根、判别式及退化方程求解；系数绝对值不超过 10¹⁰⁰
- 矩形、圆形和三边三角形面积/周长；统一长度单位，长度范围 10⁻⁹–10⁹
- 手动地理坐标的十进制度/度分秒转换，以及 WGS84 球面近似距离与初始方位；不读取设备位置
- 2×2/3×3 矩阵行列式、转置和逆矩阵；元素绝对值不超过 10¹²，近奇异矩阵拒绝求逆
- 固定年利率等额本息贷款估算，包含总额、利息和最多 600 期惰性还款明细
- 原有收支账本完整保留：
- 记录收入和支出原因、金额、分类与日期
- 按天查看、添加、修改和删除记录
- 按月份统计收入、支出、结余及消费种类
...
│   ├── resistor_decoder/     # 色环与数字贴片电阻解码
│   ├── quadratic_solver/     # 固定二次方程与退化求解
│   ├── geometry_calculator/  # 矩形、圆与三边三角形
│   ├── coordinate_tool/      # 地理坐标格式、距离与初始方位
│   ├── matrix_calculator/    # 2/3 阶行列式、转置与逆矩阵
│   └── loan_calculator/      # 固定利率等额本息与还款计划
